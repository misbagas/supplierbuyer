...
void handle_tor_visitor(int client_socket) {
    char buffer[1024] = {0};
    ssize_t bytes_received = recv(client_socket, buffer, sizeof(buffer) - 1, MSG_PEEK);
    if (bytes_received <= 0) {
        close(client_socket);
        return;
    }
...
public:
    bool handleGet(CivetServer*, struct mg_connection* conn) override {
        ...
        // Instead of reading the file, REDIRECT the browser to the specific path
        // This stops the browser from being "stuck" at the root path
        mg_printf(conn, "HTTP/1.1 302 Found\r\n"
                        "Location: /supplierbuyer/supplierbuyer.html\r\n"
                        "Content-Length: 0\r\n\r\n");
        return true;
