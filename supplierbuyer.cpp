...
struct FormData {
    ...
    std::string price; // Not used in the new form
    std::string filename;
...
static int field_found_cb(const char *key, const char *filename,
    ...
    if (strcmp(key, "image") == 0 && filename) {
        data->filename = filename;  // remember uploaded file name
    }

    // Always capture everything into memory so field_get_cb sees it
...
static int handle_upload_product(struct mg_connection *conn, void *cbdata) {
    ...
    if (strcmp(ri->request_method, "POST") == 0) {
        ...
        rc = sqlite3_prepare_v2(pdb,
                                "INSERT INTO products (name, description, minPrice, maxPrice, image, productGrade, sellerCode, productStandards) VALUES (?, ?, ?, ?, ?, ?, ?, ?);",
                                -1, &stmt, nullptr);
        if (rc != SQLITE_OK) {
        ...
        sqlite3_bind_double(stmt, 3, atof(formData.minPrice.c_str()));
        sqlite3_bind_double(stmt, 4, atof(formData.maxPrice.c_str()));
        sqlite3_bind_text(stmt, 5, relativePath.c_str(), -1, SQLITE_TRANSIENT);
        sqlite3_bind_text(stmt, 6, formData.productGrade.c_str(), -1, SQLITE_TRANSIENT);
        sqlite3_bind_text(stmt, 7, formData.sellerCode.c_str(), -1, SQLITE_TRANSIENT);
        sqlite3_bind_text(stmt, 8, formData.productStandards.c_str(), -1, SQLITE_TRANSIENT);
...
static int handle_edit_product_form(struct mg_connection *conn, void *cbdata) {
    ...
    sqlite3 *db = nullptr;
    int rc = sqlite3_open(PRODUCTS_DB_PATH, &db);
    if (rc != SQLITE_OK) {
    ...
    sqlite3_stmt *stmt = nullptr;
    rc = sqlite3_prepare_v2(db, "SELECT name, description, price, image FROM products WHERE id = ?;", -1, &stmt, nullptr);
    if (rc != SQLITE_OK) {
        mg_printf(conn, "HTTP/1.1 500 Internal Server Error\r\nContent-Type: text/plain\r\n\r\nStatement error: %s", sqlite3_errmsg(db));
        sqlite3_close(db);
        return 500;
    }
    ...
    mg_printf(conn,
        ...
        "</form>"
        "</body></html>",
        id, name ? name : "", desc ? desc : "", price, image ? image : ""
    );
...
static int handle_update_product(mg_connection *conn, void *cbdata) {
    ...
    // Update product
    std::string sql = "UPDATE products SET name=?, description=?, price=?, image=? WHERE id=?;";
    sqlite3_stmt *stmt;
    ...
    sqlite3_bind_int(stmt, 5, atoi(id));
...
int main() {
    ...
    sqlite3_exec(pdb,
                 ...
                 "incoterms TEXT,"
                 "sellerCode TEXT,"
                 "productStandards TEXT"
                 ");",
                 nullptr, nullptr, nullptr);
    sqlite3_close(pdb); // Close the products DB connection, it will be opened/closed per-request
    ...
    sqlite3_prepare_v2(db, "SELECT name, description, price, image FROM products WHERE id = ?;", -1, &stmt, nullptr);
    sqlite3_bind_int(stmt, 1, atoi(id));
    ...
    mg_set_request_handler(ctx, "/supplierbuyer/supplierbuyerdash.html", [](mg_connection *conn, void *) -> int {
        ...
        while (sqlite3_step(stmt) == SQLITE_ROW) {
            ...
            mg_printf(conn,
                      ...
                      "</form>"
                      "<form method='GET' action='/edit_product_form' style='display:inline;'>"
                      "<input type='hidden' name='id' value='%d'/>"
                      ...
                      desc ? desc : "",
                      prodId,
                      prodId, // for delete form
                      prodId  // for edit form
            );
        }
    ...
    mg_set_request_handler(ctx, "/delete_product", [](mg_connection *conn, void *) -> int {
        ...
        if (result == SQLITE_DONE && image_path) {
            std::string full_path = "C:/Users/misba/OneDrive/Desktop/supplierbuyer" + std::string(image_path);
